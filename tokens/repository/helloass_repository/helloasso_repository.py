...
class hello_asso_repository:
    ...
    def _load_access_token(self):
        ...
        logger.debug(f"Access token request with POST request to url {token_route_url} with client_id={self.client_id} and data={data}")

        r=requests.post(token_route_url,data)

        if r.status_code==200:
            logger.info("The request succeed with code 200")
            json_body=r.json()
            if "access_token" in json_body:
                logger.info("An access token has been successfully returned by the endpoint")
                return json_body["access_token"]
        else:
            logger.error(f"The server respond with statuscode={r.status_code}")
            logger.debug(f"debug:{r.text}")
            exit()

    def _load_refresh_token_from_env(self,refresh_token_path):
        logger.debug(f"Check if {refresh_token_path} exists")
        if not os.path.exists(refresh_token_path):
            logger.error(f"The file {refresh_token_path} containing refresh token does not exists")
            exit(1)
        logger.info(f"The file {refresh_token_path} exists. ")
        f = open(refresh_token_path,"r")
        refresh_token=f.read()
        f.close()

        if len(refresh_token) ==0 :
            logger.error(f"No refresh token in {refresh_token_path}")
            exit(1)
    ...
    def get_members(self,membershipslug):
        ...
        logger.debug(f"Get members by requesting {complete_url} with headers={headers}")
        ...
        if r.status_code == 200:
            ...
            logger.error(f"Error details : {r.text}")
            exit(1)